...
- Pi-native `quick_edit` tool (renamed from `fast_apply`) using Morph semantic merge. Default file editor; `edit` remains as fallback for trivial exact replacements and `write` is preferred for new files / full overwrites.
